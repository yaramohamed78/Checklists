...
//    var icons = [UIImage]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        let cell = tableView.dequeueReusableCell(withIdentifier: "iconCell", for: indexPath) as! IconCell
         ...
         configureDescription(for: cell, with: iconItem)
         configureIconImage(for: cell, with: iconItem)
        return cell
    }
    
    func configureDescription(for cell : IconCell , with item : Icon)
    {
        let descriptionLabel = cell.iconDescription
        descriptionLabel?.text = item.description
    
    }
    
    func configureIconImage(for cell : IconCell , with item : Icon)
    {
        let iconImage = cell.iconImage
    ...
    func loadTableView()
    {
        ...
        {
            let newIcon = Icon()
            newIcon.image = images[i]
            newIcon.description = descriptions[i]
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        if let cell = tableView.cellForRow(at: indexPath)
        {
            let iconItem = icons[indexPath.row]
            print("Here")
            delegate?.iconsViewControllerDelegate(self, didFinishAdding: iconItem)
